<div class="space-y-6">
        ...
        <div class="flex items-start gap-3 px-4 py-3 rounded-xl
            ...
            <div>
                ...
                <p class="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                    ...
                    se convertirán en equipos principales y seguirán prestados al usuario.
    ...
    <div class="bg-white dark:bg-cerberus-mid
        ...
        {{-- Items agrupados por préstamo --}}
        @php
            $itemsPorPrestamo = $this->todosLosItemsActivos->groupBy('prestamo_id');
        @endphp

        @forelse ($itemsPorPrestamo as $prestamoId => $items)
            @php
                $primerPrestamo = $items->first()->prestamo;
            @endphp

            {{-- Separador de préstamo (solo si hay más de uno) --}}
            @if ($itemsPorPrestamo->count() >= 1)
                <div class="px-5 py-2 bg-gray-50 dark:bg-cerberus-dark/60
                    ...
                    <span class="text-xs text-gray-500 dark:text-cerberus-accent font-medium">
                        Préstamo #{{ $prestamoId }}
                        · {{ $primerPrestamo?->fecha_prestamo?->format('d/m/Y') ?? '—' }}
                        · {{ $primerPrestamo?->empresa?->nombre ?? '—' }}
                        @if ($primerPrestamo)
                            <x-prestamos.badge-estado :estado="$primerPrestamo->estado" />
                        @endif
                    </span>
                </div>
            @endif

            {{-- Items de este préstamo --}}
        ...
        @empty
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-cerberus-steel">
                    Este usuario no tiene equipos en préstamo activos.
                </p>
            </div>
        @endforelse
    ...
    <div class="flex items-center justify-between gap-4
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.prestamos.historial', $this->usuarioId) }}"
               wire:navigate

<div class="space-y-5">
    ...
    <div class="flex items-center gap-0">
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm {{ $paso === 1 ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-400 dark:text-cerberus-steel' }}">Receptor</span>
        </div>
        <div class="flex-1 h-px mx-3 {{ $paso > 1 ? 'bg-cerberus-primary' : 'bg-gray-200 dark:bg-cerberus-steel/40' }}"></div>
        <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold
                        {{ $paso === 2 ? 'bg-cerberus-primary text-white' : 'bg-gray-200 dark:bg-cerberus-steel/40 text-gray-500 dark:text-cerberus-steel' }}">2</div>
            <span class="text-sm {{ $paso === 2 ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-400 dark:text-cerberus-steel' }}">Equipos</span>
        </div>
    </div>

    @error('general')
        <div class="flex items-center gap-2 px-4 py-3 rounded-lg bg-red-50 dark:bg-red-900/30
                    border border-red-200 dark:border-red-700/50 text-red-700 dark:text-red-300 text-sm">
            <span class="material-icons text-base flex-shrink-0">error_outline</span> {{ $message }}
        </div>
    @enderror
    ...
    @if ($paso === 1)
        <div class="bg-white dark:bg-cerberus-mid border border-gray-200 dark:border-cerberus-steel
            ...
            <h2 class="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                <span class="material-icons text-cerberus-accent text-lg">swap_horiz</span>
                ¿A quién se presta el equipo?
            </h2>
            ...
            <div class="flex gap-3">
                <button type="button" wire:click="$set('tipo_receptor', 'usuario')"
                    ...
                    <div class="text-left">
                        ...
                        <p class="text-xs opacity-60">Préstamo a un usuario</p>
                    </div>
                </button>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <x-form.input type="date" label="Fecha de préstamo *" wire:model="fecha_prestamo" />
                    @error('fecha_prestamo') <p class="text-xs text-red-500 -mt-3 mb-2">{{ $message }}</p> @enderror
                </div>
                <div>
                    <x-form.input type="date" label="Fecha de devolución esperada" wire:model="fecha_devolucion_esperada"
                        hint="Opcional. Si se establece, el sistema marcará el préstamo como Vencido si pasa sin devolución." />
                </div>
            </div>

            <x-form.textarea label="Observaciones" wire:model="observaciones" placeholder="Motivo del préstamo, notas adicionales..." rows="2" />
    ...
    @if ($paso === 2)
        <div class="flex gap-5 items-start animate-fade-in">
            ...
            <div class="flex-1 min-w-0 space-y-4">
                ...
                <div class="bg-white dark:bg-cerberus-mid border border-gray-200 dark:border-cerberus-steel rounded-xl p-4 space-y-3">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        <div class="flex rounded-lg border border-gray-200 dark:border-cerberus-steel/50 overflow-hidden flex-shrink-0">
                            <button type="button" wire:click="toggleVistaEquipos"
                                class="flex items-center gap-1.5 px-3 py-2 text-xs font-medium transition
                                       {{ $vistaEquipos === 'grilla' ? 'bg-cerberus-primary text-white' : 'bg-white dark:bg-cerberus-dark text-gray-600 dark:text-cerberus-light hover:bg-gray-50' }}">
                                <span class="material-icons text-base">apps</span>
                                <span class="hidden sm:inline">Grilla</span>
                            </button>
                            <button type="button" wire:click="toggleVistaEquipos"
                                class="flex items-center gap-1.5 px-3 py-2 text-xs font-medium transition border-l border-gray-200 dark:border-cerberus-steel/50
                                       {{ $vistaEquipos === 'lista' ? 'bg-cerberus-primary text-white' : 'bg-white dark:bg-cerberus-dark text-gray-600 dark:text-cerberus-light hover:bg-gray-50' }}">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <button type="button" wire:click="setCategoriaFiltro('')"
                            class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition
                                   {{ $filtro_categoria === '' ? 'bg-cerberus-primary border-cerberus-primary text-white' : 'bg-gray-50 dark:bg-cerberus-dark border-gray-200 dark:border-cerberus-steel/60 text-gray-600 dark:text-cerberus-light hover:border-cerberus-primary' }}">
                            <span class="material-icons text-xs">apps</span> Todas
                        </button>
                        @foreach ($this->categorias as $cat)
                            <button type="button" wire:click="setCategoriaFiltro('{{ $cat->id }}')"
                                class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition
                                       {{ $filtro_categoria === (string) $cat->id ? 'bg-cerberus-primary border-cerberus-primary text-white' : 'bg-gray-50 dark:bg-cerberus-dark border-gray-200 dark:border-cerberus-steel/60 text-gray-600 dark:text-cerberus-light hover:border-cerberus-primary' }}">
                                {{ $cat->nombre }}
                                <span class="px-1.5 py-0.5 rounded-full text-xs {{ $filtro_categoria === (string) $cat->id ? 'bg-white/25 text-white' : 'bg-gray-200 dark:bg-cerberus-steel/50 text-gray-600 dark:text-cerberus-light' }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                            @foreach ($this->equiposDisponibles as $equipo)
                                @php
                                    ...
                                    $atvsVisible = $equipo->atributosActuales->filter(fn($v) => $v->atributo?->visible_en_tabla)->take(3);
                                @endphp
                                <div class="bg-white dark:bg-cerberus-mid
                                            border {{ $estaEnCarrito ? 'border-cerberus-primary/60' : 'border-gray-200 dark:border-cerberus-steel' }}
                                            rounded-xl p-4 flex flex-col gap-3 transition">
                                    <div class="flex items-start gap-2">
                                        <span class="material-icons text-2xl text-cerberus-accent flex-shrink-0">devices</span>
                                        <div class="min-w-0">
                                            ...
                                            @if ($equipo->ubicacion)
                                                <p class="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                                                    <span class="material-icons text-xs">location_on</span>{{ $equipo->ubicacion->nombre }}
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                                    <button type="button" wire:click="agregarAlCarrito({{ $equipo->id }})" @disabled($estaEnCarrito)
                                        class="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-medium transition
                                               {{ $estaEnCarrito ? 'bg-cerberus-primary/10 text-cerberus-primary dark:text-cerberus-accent cursor-default' : 'bg-cerberus-primary hover:bg-cerberus-hover text-white' }}">
                                        <span class="material-icons text-sm">{{ $estaEnCarrito ? 'check_circle' : 'add_shopping_cart' }}</span>
                        ...
                        <div class="bg-white dark:bg-cerberus-mid border border-gray-200 dark:border-cerberus-steel rounded-xl overflow-hidden">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-50 dark:divide-cerberus-steel/20">
                                    @foreach ($this->equiposDisponibles as $equipo)
                                        @php $estaEnCarrito = collect($carrito)->contains('id', $equipo->id); @endphp
                                            ...
                                            <td class="px-4 py-2.5 text-center">
                                                <button type="button" wire:click="agregarAlCarrito({{ $equipo->id }})" @disabled($estaEnCarrito)
                                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition
                                                           {{ $estaEnCarrito ? 'bg-cerberus-primary/10 text-cerberus-primary cursor-default' : 'bg-cerberus-primary hover:bg-cerberus-hover text-white' }}">
                                                    <span class="material-icons text-sm">{{ $estaEnCarrito ? 'check' : 'add' }}</span>
            ...
            <div class="w-80 flex-shrink-0 sticky top-20 space-y-3">
                ...
                <div class="bg-white dark:bg-cerberus-mid border border-gray-200 dark:border-cerberus-steel rounded-xl p-4">
                    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-cerberus-steel mb-2">Prestando a</p>
                    <div class="flex items-center gap-2">
                        <span class="material-icons text-base text-cerberus-accent">{{ $tipo_receptor === 'usuario' ? 'person' : 'corporate_fare' }}</span>
                        <span class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ $this->receptorNombre }}</span>
                    </div>
                    @if ($fecha_devolucion_esperada)
                        <p class="text-xs text-amber-500 dark:text-amber-400 mt-1.5 flex items-center gap-1">
                            <span class="material-icons text-xs">schedule</span>
                            Devolver: {{ \Carbon\Carbon::parse($fecha_devolucion_esperada)->format('d/m/Y') }}
                        </p>
                    @endif
                    <button type="button" wire:click="volverPaso1" class="mt-1.5 text-xs text-cerberus-primary hover:text-cerberus-hover transition">
                        ← Cambiar datos
                    </button>
                </div>
                ...
                <div class="bg-white dark:bg-cerberus-mid border border-gray-200 dark:border-cerberus-steel rounded-xl overflow-hidden">
                    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-cerberus-steel/50">
                        ...
                        @if (count($carrito) > 0)
                            <span class="px-2 py-0.5 text-xs rounded-full bg-cerberus-primary text-white font-medium">{{ count($carrito) }}</span>
                        @endif
                    </div>
                    ...
                    <div class="max-h-80 overflow-y-auto divide-y divide-gray-50 dark:divide-cerberus-steel/20">
                        @forelse ($carrito as $item)
                            <div wire:key="carrito-{{ $item['uid'] }}" class="px-4 py-3 space-y-2">
                                <div class="flex items-center gap-2">
                                    <span class="material-icons text-base text-cerberus-accent flex-shrink-0">{{ $item['icono'] }}</span>
                                ...
                                @if (count($this->itemsPrincipalesCarrito) > 1 ||
                                    ...
                                    <select wire:change="setPadre('{{ $item['uid'] }}', $event.target.value)"
                                        class="w-full text-xs rounded-lg px-2 py-1.5 bg-gray-50 dark:bg-cerberus-dark
                                        ...
                                        @foreach ($this->itemsPrincipalesCarrito as $principal)
                                            @if ($principal['uid'] !== $item['uid'])
                                                <option value="{{ $principal['uid'] }}" {{ $item['padre_uid'] === $principal['uid'] ? 'selected' : '' }}>
                        ...
                        @empty
                            <div class="flex flex-col items-center justify-center py-8 px-4 text-center">
                                ...
                                <p class="text-xs text-gray-400 dark:text-cerberus-steel">Selecciona equipos de la grilla</p>
                            </div>
                        @endforelse
                    </div>
                    ...
                    <div class="p-4 border-t border-gray-100 dark:border-cerberus-steel/50">
                        <button type="button" wire:click="confirmar" wire:loading.attr="disabled" @disabled(empty($carrito))
                            ...
                            <span wire:loading.remove wire:target="confirmar">Confirmar @if (count($carrito) > 0)({{ count($carrito) }})@endif</span>

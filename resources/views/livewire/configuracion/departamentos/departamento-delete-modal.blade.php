        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative z-50 w-full max-w-md mx-4 p-6 bg-white dark:bg-cerberus-mid
                ...
                <p class="text-sm text-gray-600 dark:text-cerberus-light mb-1">
                    ¿Deseas desactivar
                    <strong class="text-gray-900 dark:text-white">{{ $departamento->nombre }}</strong>?
                </p>
                ...
                @if ($totalUsuarios > 0)
                    <div class="mt-3 px-4 py-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/40
                        ...
                        <span>
                            ...
                            <strong>{{ $totalUsuarios }} usuario(s) activo(s)</strong> en este departamento.
                        </span>
                    </div>
                @else
                    <div class="mt-3 px-4 py-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/40
                                rounded-lg text-sm text-yellow-700 dark:text-yellow-400 space-y-1">
                        <div class="flex items-start gap-2">
                            <span class="material-icons text-base mt-0.5">info</span>
                            <span>
                                El departamento dejará de aparecer en los formularios, pero
                                <strong>no se eliminará</strong>. Podrás reactivarlo desde esta tabla.
                            </span>
                        </div>
                        @if ($totalCargos > 0)
                            <p class="pl-6 text-xs">
                                También se desactivarán los
                                <strong>{{ $totalCargos }} cargo(s)</strong> de este departamento que no tengan usuarios activos.
                            </p>
                        @endif
                    </div>
                @endif

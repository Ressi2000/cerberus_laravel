            ...
            <div class="relative z-50 w-full max-w-md mx-4 bg-white dark:bg-cerberus-mid
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-cerberus-steel">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        <span class="material-icons text-cerberus-accent">{{ $tipoLicenciaId ? 'edit' : 'add_circle' }}</span>
                        {{ $tipoLicenciaId ? 'Editar tipo de licencia' : 'Nuevo tipo de licencia' }}
                    </h2>
                ...
                <div class="px-6 py-5">
                    <x-form.input
                        label="Nombre"
                        wire:model="nombre"
                        placeholder="Ej: Premium, Standard, Basic, Outlook simple, No tiene..."
                        :error="$errors->first('nombre')"
                        required
                        hint="El nombre debe ser único. Se usará en el select de licencias del módulo de usuarios."
                    />
                </div>

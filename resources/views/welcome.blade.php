...
<body
    ...
    <!-- HERO PRINCIPAL - FONDO CON EL GIF -->
    <section class="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <header
            class="absolute top-0 left-0 right-0 z-20 w-full px-8 py-6 flex justify-between items-center max-w-7xl mx-auto">
            <div class="flex items-center gap-3">
                <picture>
                    ...
                    <img src="{{ asset('images/cerberusLight.png') }}" alt="Cerberus Logo"
                        class="h-12 w-auto transition-all duration-500 brightness-0 invert">
                </picture>
                ...
                <div class="flex flex-wrap justify-center gap-4 pt-4">
                    ...
                    <a href="#features"
                        class="px-8 py-4 border-2 border-white/30 hover:border-white/60
                              ...
                              backdrop-blur-sm hover:scale-105 transform duration-300">
                        Conocer más ↓
                    </a>
                </div>
            </div>
    ...
    <section id="features" class="py-24 w-full relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-7xl mx-auto text-center px-6">
            <div class="mb-16">
                <span
                    class="inline-block px-4 py-2 rounded-full bg-[#A9D6E5]/10 border border-[#A9D6E5]/20 text-[#A9D6E5] text-sm font-medium mb-4 backdrop-blur-sm">
                    🚀 Características
                </span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ([['🖥️', 'Inventario de Equipos', 'Registra y clasifica activos tecnológicos con categorías configurables, estados, ubicaciones y atributos dinámicos por tipo de equipo (RAM, disco, S/N, etc.).'], ['📋', 'Asignaciones', 'Asigna equipos a usuarios o áreas comunes de forma permanente. Soporta periféricos vinculados al equipo principal con devoluciones independientes.'], ['🔄', 'Préstamos', 'Gestiona préstamos temporales con fechas de vencimiento, alertas de expiración, renovaciones y seguimiento de devoluciones.'], ['🚚', 'Traslados', 'Registra movimientos físicos de equipos entre ubicaciones con numeración automática (TRA-YYYY-NNN) y documentación del proceso.'], ['🔍', 'Auditoría', 'Cada acción queda registrada: quién la realizó, cuándo, y qué cambió. Trazabilidad completa para cumplimiento y control.'], ['⚙️', 'Configuración', 'Administra categorías, estados, ubicaciones, departamentos, cargos y empresas desde un panel centralizado.']] as [$icon, $title, $desc])
                    <div
                        class="group p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-[#A9D6E5]/40 hover:bg-white/10 transition-all duration-500 text-left hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#A9D6E5]/5">
                        <div
                            class="text-5xl mb-4 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500">
                            {{ $icon }}</div>
                        <h4
                            class="text-xl font-semibold mb-3 text-[#A9D6E5] group-hover:text-white transition-colors duration-300">
                            {{ $title }}</h4>
                        <p
    ...
    <section class="py-24 w-full relative bg-[#F0F4F8] dark:bg-[#0D1B2A] transition-colors duration-500">
        <div class="relative z-10 max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach ([['Administrador', 'Control total del sistema: configuración, usuarios, empresas y todos los datos del inventario.', 'border-[#1E40AF]/20 dark:border-red-500/30 bg-[#1E40AF]/5 dark:bg-red-500/5', '👑', 'text-[#1E40AF] dark:text-red-400'], ['Analista', 'Gestión operativa de equipos, asignaciones, préstamos y traslados en su contexto de empresa.', 'border-[#1E40AF]/20 dark:border-blue-500/30 bg-[#1E40AF]/5 dark:bg-blue-500/5', '📊', 'text-[#1E40AF] dark:text-blue-400'], ['Usuario', 'Visualización del inventario asignado y préstamos activos asociados a su perfil.', 'border-[#1E40AF]/20 dark:border-green-500/30 bg-[#1E40AF]/5 dark:bg-green-500/5', '👤', 'text-[#1E40AF] dark:text-green-400']] as [$rol, $desc, $style, $emoji, $color])
                    <div
                        class="group p-8 rounded-2xl border {{ $style }} bg-white/80 dark:bg-transparent backdrop-blur-sm hover:-translate-y-3 transition-all duration-500 hover:shadow-xl">
                        <div class="text-5xl mb-4 group-hover:scale-110 transition-transform duration-500">
                            {{ $emoji }}</div>
                        <h4
                            class="font-bold text-2xl mb-3 {{ $color }} group-hover:text-[#0D1B2A] dark:group-hover:text-white transition-colors duration-300">
                            {{ $rol }}</h4>
                        <p
                            ...
                            {{ $desc }}</p>
                        <div
                            class="mt-6 h-0.5 w-12 bg-[#1E40AF]/20 dark:bg-white/20 group-hover:w-full group-hover:bg-[#1E40AF] dark:group-hover:bg-[#A9D6E5]/50 transition-all duration-500 rounded-full">
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-wrap justify-center gap-4">
                        <a href="{{ route('login') }}"
                            class="group inline-flex items-center gap-2 px-10 py-5 bg-[#1E40AF] hover:bg-[#1E3A8A] dark:bg-[#A9D6E5] dark:hover:bg-[#89C2D9]
                              text-white dark:text-[#0D1B2A] rounded-lg font-semibold text-xl transition-all duration-300 shadow-lg
                              hover:scale-105 hover:shadow-2xl hover:shadow-[#1E40AF]/20 dark:hover:shadow-[#A9D6E5]/20">
                            Comenzar ahora
                            <span class="group-hover:translate-x-1 transition-transform duration-300">→</span>
                        </a>
                        <a href="#features"
                            class="px-10 py-5 border-2 border-[#1E40AF]/30 dark:border-white/30 hover:border-[#1E40AF]/50 dark:hover:border-[#A9D6E5]/50
                              text-[#1E40AF] dark:text-white hover:text-[#0D1B2A] dark:hover:text-[#A9D6E5] rounded-lg font-semibold text-xl transition-all duration-300
                              backdrop-blur-sm hover:scale-105 hover:shadow-xl hover:bg-white/50 dark:hover:bg-white/5">

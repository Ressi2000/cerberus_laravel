...
<aside id="sidebar"
    ...
    {{-- ── LOGO ───────────────────────────────────────────────────────────── --}}
    <div class="flex items-center flex-shrink-0" style="height:56px; min-height:56px;">
        ...
        <div
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 px-3 w-full min-w-0">
                ...
                {{-- Logo modo OSCURO --}}
                <img src="{{ asset('images/logos/crb2-dark.png') }}" alt="Cerberus"
                    class="hidden dark:block flex-shrink-0 h-8 w-8 object-contain">

                {{-- Logo modo CLARO --}}
                <img src="{{ asset('images/logos/crb2-ligth.png') }}" alt="Cerberus"
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-3 px-2 space-y-0.5">
        ...
        {{-- ── DASHBOARD ──────────────────────────────────────────────────── --}}
        <a href="{{ route('dashboard') }}" class="{{ $li }} {{ $active('dashboard') ? $on : $off }}"
            title="Dashboard">
        ...
        <div x-data="{ open: {{ $usersOpen ? 'true' : 'false' }} }">
            ...
            <button @click="open = !open" class="{{ $li }} {{ $usersOpen ? $on : $off }}" title="Usuarios">
                ...
                <span
                    class="sidebar-arrow material-icons text-sm
        ...
        <div x-data="{ open: {{ $eqOpen ? 'true' : 'false' }} }">
            ...
            <button @click="open = !open" class="{{ $li }} {{ $eqOpen ? $on : $off }}" title="Equipos">
        ...
        <a href="{{ route('admin.auditoria.index') }}"
            class="{{ $li }} {{ $active('admin.auditoria.*') ? $on : $off }}" title="Auditoría">
        ...
        @role('Administrador')
            ...
            <div x-data="{ open: {{ $cfgOpen ? 'true' : 'false' }} }">
                ...
                <button @click="open = !open" class="{{ $li }} {{ $cfgOpen ? $on : $off }}"
                    title="Configuración">
                    ...
                    @include('components.ui._sidebar-tooltip', ['label' => 'Configuración'])
                </button>
                ...
                <div class="sidebar-submenu mt-0.5 ml-4 pl-3
                    ...
                    <a href="{{ route('admin.configuracion.categorias') }}"
                        class="{{ $li }} py-1.5 text-sm
                          {{ $active('admin.configuracion.categorias') ? $on : $off }}">
                        <span
                            class="material-icons text-base flex-shrink-0
                                 {{ $active('admin.configuracion.categorias') ? $ion : $ioff }}">
                            category
                        </span>
                        <span class="whitespace-nowrap">Categorías</span>
                    </a>

                    <a href="{{ route('admin.configuracion.estados') }}"
                        class="{{ $li }} py-1.5 text-sm
                          {{ $active('admin.configuracion.estados') ? $on : $off }}">
                        <span
                            class="material-icons text-base flex-shrink-0
                                 {{ $active('admin.configuracion.estados') ? $ion : $ioff }}">
                            flag
                        </span>
                        <span class="whitespace-nowrap">Estados</span>
                    </a>

                    <a href="{{ route('admin.configuracion.atributos') }}"
                        class="{{ $li }} py-1.5 text-sm
                          {{ $active('admin.configuracion.atributos') ? $on : $off }}">
                        <span
                            class="material-icons text-base flex-shrink-0
                                 {{ $active('admin.configuracion.atributos') ? $ion : $ioff }}">
                            tune
                        </span>
    ...
    <div class="flex-shrink-0 border-t border-gray-200 dark:border-cerberus-steel/50 p-2">
        ...
        <p
            class="sidebar-logo-text text-[10px] text-gray-400 dark:text-cerberus-steel/60
...
{{-- Backdrop móvil --}}
<div id="sidebar-backdrop" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-30 lg:hidden hidden">
</div>
